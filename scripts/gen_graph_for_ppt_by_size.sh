...
do
    ...
    if "$read_f"
    then
	graph[$s]=$'set title "C-'$title'-R"\n'
    elif "$write_f"
    then
	graph[$s]=$'set title "C-'$title'-W"\n'
    else
	graph[$s]=$'set title "C-'$title$'"\n'
    fi
...
set terminal postscript eps enhanced color size 10in,10in font 'Helvetica,20' linewidth 2
set output "${eps_out}"
set multiplot layout 2,2 title "Cumulative Memory Usage by Policy ${title}" font 'Helvetica,22'
